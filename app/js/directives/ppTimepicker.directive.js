(function(angular) {
    "use strict";
    angular.module('directive.timepicker', ["dateParser"])
        .factory("mdTimepickerHelpers", function() {
            return {
        ...
        .directive("mdTimepicker", ["$compile", "$parse", "$document", "dateFilter", "$dateParser", "mdTimepickerHelpers", "$log", function($compile, $parse, $document, dateFilter, $dateParser, mdTimepickerHelpers, $log) {
            return {
                restrict: "A",
                require: "ngModel",
                scope: {
                ...
                link: function(scope, element, attrs, ctrl) {
                    ...
                    scope.timepicker = {
                        ...
                        optionList: function() {
                            if (updateList) {
                                list = mdTimepickerHelpers.buildOptionList(scope.timepicker.minTime, scope.timepicker.maxTime, scope.timepicker.step);
                                updateList = false;
                            }
                            return list;
                        }
                    ...
                    attrs.$observe("step", function(value) {
                        if (!value) return;
                        var step = mdTimepickerHelpers.stringToMinutes(value);
                    ...
                    scope.openPopup = function() {
                        scope.position = angular.extend({}, element[0].getBoundingClientRect());
                        scope.position.top = scope.position.top + element.prop("offsetHeight");
                        scope.timepicker.isOpen = true;
                        scope.timepicker.activeIdx = mdTimepickerHelpers.getClosestIndex(scope.ngModel, scope.timepicker.optionList());
                        scope.$digest();
                    ...
                    element.after($compile(angular.element("<div md-timepicker-popup></div>"))(scope));
        ...
        .directive("mdTimepickerPopup", function() {
            return {
                restrict: "A",
                replace: true,
                template: '<div class="md-timepicker-popup" ng-show="timepicker.isOpen"><md-list>' +
                    '<md-list-item md-timepicker-item class="md-clickable" ng-class="{active: isActive($index)}" ng-repeat="time in timepicker.optionList()" ng-click="select(time, $index)">' +
                    '{{time | date:timepicker.timeFormat}}' +
                ...
                link: function(scope, element, attrs) {
                    ...
                    scope.scrollToSelected = function() {
                        if (scope.timepicker.activeIdx > -1) {
                            element[0].scrollTop = (scope.timepicker.activeIdx * 48) - 50;
                        }
                    };
                }
            };
